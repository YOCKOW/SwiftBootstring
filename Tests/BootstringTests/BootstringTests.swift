...
// TODO?: Add tests other than Punycode.
...
@Test("Punycode Decoding Test", arguments: punycodeSamples)
func testPunycodeDecoding(_ pair: (original: String, encoded: String)) throws {
  let expected = pair.original
  let encoded = pair.encoded

  let decoded: String? = encoded.removingPunycodeEncoding
  #expect(try #require(decoded) == expected)
}

@Test("Punycode Encoding Test", arguments: punycodeSamples)
func testPunycodeEncoding(_ pair: (original: String, encoded: String)) throws {
  let original = pair.original
  let expected = pair.encoded

  let encoded: String? = original.addingPunycodeEncoding
  #expect(try #require(encoded) == expected)
}
#else
final class BootstringTests: XCTestCase, @unchecked Sendable {
  func testPunycodeDecoding() {
    for ii in 0..<punycodeSamples.count {
      let expected = punycodeSamples[ii].original
      let encoded = punycodeSamples[ii].encoded
  ...
  func testPunycodeEncoding() {
    for ii in 0..<punycodeSamples.count {
      let original = punycodeSamples[ii].original
      let expected = punycodeSamples[ii].encoded
